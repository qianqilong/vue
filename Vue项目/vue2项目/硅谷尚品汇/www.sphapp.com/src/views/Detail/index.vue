		...
		<section class="con">
			...
			<div class="mainCon">
				...
				<div class="previewWrap">
					...
					<!-- 小图列表 -->
					<ImageList :skuImageArr="skuImageList" />
				</div>
				<!-- 右侧选择区域布局 -->
				<div class="InfoWrap">
						...
						<div class="chooseArea">
							...
							>
								...
								<dd
									...
									@click="changeActive(item,item2.id)"
...
export default {
	...
	methods: {
    // 改变商品属性的选择状态
		changeActive(obj, item) {
			obj.spuSaleAttrValueList.forEach(element => {
				element.isChecked = 0
				if (element.id == item) {
					element.isChecked = 1
				}
    ...
    // 约束购物输入框的数据
    changeskuNum(){
      if(this.goodNum<=1)this.goodNum=1
      else this.goodNum=parseInt(this.goodNum)
    },
    // 点击加入购物车按键
   async addShopcar(){
    const res=  await this.$store.dispatch('addShopCart', { skuId:this.skuInfo.id, skuNum:this.goodNum} )
    if(res.code==200){
			// 成功跳转,存入会话存储中
		sessionStorage.setItem('SKUINFO',JSON.stringify(this.skuInfo))
     this.$router.push(`/addcartsuccess?skuNum=${this.goodNum}`)
    }else{
      console.log('err');
    }
   }
	...
	computed: {
		...
		//  给子组件数据是仓库中的，可能是一个unddefine传输使用会有警告
		skuImageList() {
